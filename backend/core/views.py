...
from .serializers import (
    NewsletterSubscriptionSerializer, SportCategorySerializer,
    NewsArticleSerializer, MatchFixtureSerializer, NewsletterSerializer,
    UnsubscribeSerializer, PreferencesUpdateSerializer, NewsletterSubscriberSerializer
)
from core.tasks import send_newsletter_to_subscribers, track_email_open
...
def newsletter_preferences(request, token):
    ...
    if request.method == 'GET':
        ...
        sport_categories = SportCategorySerializer(
            SportCategory.objects.filter(is_active=True), 
            many=True
        )
        
        return Response({
            'subscriber': serializer.data,
            'available_sports': sport_categories.data
        })
...
def latest_newsletter(request):
    ...
    try:
        ...
        return Response(
            {'message': 'No newsletters available'}, 
            status=status.HTTP_404_NOT_FOUND
        )
